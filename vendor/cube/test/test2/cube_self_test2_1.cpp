...
/**
 * \file cube_test.cpp
 * \brief An example how to create with advanced system tree and save a cube.
 */
#include <iostream>
#include <string>
#include <vector>
...
main( int argc, char* argv[] )
{
    ...
    SystemTreeNode* mach, * node;
    LocationGroup*  proc0, * proc1;
    Location*       thrd0, * thrd1;

    std::string cubename = "example2_2";
    try
    {
        Cube cube;
//   Cube cube;
        ...
        // Build location tree
        mach = cube.def_system_tree_node( "MSC", "my machine", "machine" );
        node = cube.def_system_tree_node( "Athena", "my node", "node", mach );

        proc0 = cube.def_location_group( "Process 0", 0, CUBE_LOCATION_GROUP_TYPE_PROCESS, mach );
        proc1 = cube.def_location_group( "Process 1", 1, CUBE_LOCATION_GROUP_TYPE_PROCESS, node );
        thrd0 = cube.def_location( "Thread 0", 0, CUBE_LOCATION_TYPE_CPU_THREAD, proc0 );
        thrd1 = cube.def_location( "Thread 1", 1, CUBE_LOCATION_TYPE_CPU_THREAD, proc1 );
        ...
        cube.set_sev( met0, cnode2, thrd0, 5. );
        cube.set_sev( met0, cnode2, thrd1, 9. );
        cube.set_sev( met1, cnode0, thrd0, 2. );
        cube.set_sev( met1, cnode0, thrd1, 1. );
        cube.set_sev( met1, cnode1, thrd0, 2. );
        cube.set_sev( met1, cnode1, thrd1, 1. );
        cube.set_sev( met1, cnode2, thrd0, 2. );
