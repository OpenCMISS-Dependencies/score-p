...
#ifndef __CUBE_ALGEBRA4_INTERNALS_CPP
#define __CUBE_ALGEBRA4_INTERNALS_CPP
 ...
 ********/
bool
metric_merge( Cube& newCube, const Cube& input, CubeMapping&   cubeMap )
{
    bool is_equal = true;
    /* Get roots */
    ...
    for ( size_t i = 0; i < inroot.size(); ++i )
    {
        ...
        for ( size_t j = 0; j < newroot.size(); ++j )
        {
            ...
            if ( rhs->get_uniq_name() == lhs->get_uniq_name() )
            {
                ...
                if ( !compare_tree( newCube, *lhs, *rhs, cubeMap ) )
                {
                    is_equal = false;
                }
            }
 ...
 */
bool
compare_tree( Cube& newCube, Metric& lhs,  Metric& rhs,   CubeMapping&   cubeMap )
{
    bool is_equal = true;
    for ( unsigned int i = 0; i < rhs.num_children(); ++i )
    {
        ...
        for ( unsigned int j = 0; j < lhs.num_children(); ++j )
        {
            ...
            {       /* Tree already exists? */
                ...
                {       /* Has metric children? */
                    if ( !compare_tree( newCube, *lhs.get_child( j ), *it, cubeMap ) )
                    {
                        is_equal = false;
                        break;
                    }
 ...
 */
void
copy_tree( Cube& newCube, Metric& rhs,   CubeMapping&   cubeMap,  CallTreeCopy modifier, bool omit_void )
{
    ...
    for ( unsigned int i = 0; i < rhs.num_children(); ++i )
    {
        ...
        TypeOfMetric mtype = ( modifier == COPY_SAME_FLAVOUR ) ?
        ...
        if ( met != NULL )
        {
            ...
            {
                copy_tree( newCube, *rhs.get_child( i ), cubeMap, modifier, omit_void );
            }
        }
    }
}
 ...
 */
bool
cnode_reroot( Cube& newCube, const Cube& input, string cn_newrootname,
              vector<string> cn_prunes, CubeMapping& cubeMap, bool copy_ids )
{
...
copy_tree( Cube& newCube, Cnode& rhs, Cnode& parent,
           vector<string> prunes, CubeMapping& cubeMap, bool copy_ids  )
...
def_cnode( Cube& newCube, Cnode& rhs, Cnode* parent, bool copy_ids   )
{
    ...
    for ( int i = 0; i < num; ++i )
    {
        ...
        {
            return newCube.def_cnode( newRegv[ i ], rhs.get_mod(), rhs.get_line(), parent, ( copy_ids ) ? rhs.get_id() : ( uint32_t )-1 );
        }
    }

    Region* newregion;
    newregion = newCube.def_region( rhs.get_callee()->get_name(),
    ...
    return newCube.def_cnode( newregion, rhs.get_mod(), rhs.get_line(), parent, ( copy_ids ) ? rhs.get_id() : ( uint32_t )-1  );
}
 ...
 */
bool
sysres_merge( Cube& newCube, const Cube& rhs, CubeMapping& cubeMap, bool reduce, bool collapse )
{
    if ( collapse )
    {
    ...
    if ( is_subset( rhs, newCube ) )
    {
        copy_system_tree( newCube, rhs, cubeMap );
        return true;
    }
    else
    if ( is_subset( newCube, rhs, &cubeMap  ) )
    {
...
void
merge( Cube& newCube, SystemTreeNode* target_stn, SystemTreeNode* to_copy, CubeMapping& cubeMap, CubeMapping* midCubeMap  )
{
    for ( unsigned i = 0; i < to_copy->num_children(); ++i  )
    {
    ...
    for ( std::vector<SystemTreeNode*>::iterator new_snt_iter = new_stns.begin(); new_snt_iter != new_stns.end(); /*new_snt_iter++*/ )
    {
        ...
        for ( std::vector<SystemTreeNode*>::iterator stn_iter = stns.begin(); stn_iter != stns.end(); )
        {
            ...
            if ( ( *new_stn ) == ( *stn ) )
            {
                cubeMap.stnm[ new_stn ] = stn;
                cubeMap.r_stnm[ stn ]   = new_stn;
                merge( newCube, stn, new_stn, cubeMap );
                stns.erase( stn_iter );
        ...
        if ( !equal )
        {
            ...
            cubeMap.stnm[ new_stn ]       = created_stn;
            cubeMap.r_stnm[ created_stn ] = new_stn;
            merge( newCube, created_stn, new_stn, cubeMap );
            new_stns.erase( new_snt_iter );
        }
    }
}
 ...
 */
bool
is_subset( const Cube& lhs, const Cube& rhs, CubeMapping* cubeMap )
{
    ...
    for ( vector<SystemTreeNode*>::iterator riter = inroot.begin(); riter != inroot.end(); /* riter++ */ )
    {
        ...
        {
            subset = true;
            if ( !is_subset( *liter, *riter, cubeMap ) )
            {
                return false;
            }
            else
 ...
 */
bool
is_subset( SystemTreeNode* bigger, SystemTreeNode*  smaller, CubeMapping* cubeMap  )
{
    if ( !( *bigger == *smaller ) )
    {
        return false;
...
is_subset( const vector<Node*>& big, const vector<Node*>& small, bool& is_equal )
{
    ...
    {
//         if ( small.size() == i )
//         {       /* Has the bigger experiment more nodes then break! */
//             break;
//         }
...
//     sysres_merge( newCube, rhs, true, false );
...
createTreeEnumeration( vector<T*>& list_of_vertices )
{
    ...
    for (; current_index < list_of_vertices.size(); current_index++ )
    {
        ...
        stable_sort( _v_children.begin(), _v_children.end(), hash_compare );
...
inline bool
hash_compare( IdentObject* v1, IdentObject* v2 )
{
    return v1->get_hash_id().compare( v2->get_hash_id() ) < 0;
}


bool
createFastMapping( Cube& lhsCube, Cube& rhsCube, CubeMapping& cubeMap )
{
    // ------------- map metrcs ----------------
//     cout << " Create metrics mapping " << endl;
    vector<Metric*> lhs_metrics = lhsCube.get_root_metv();
    vector<Metric*> rhs_metrics = rhsCube.get_root_metv();
    createTreeEnumeration<Metric>( lhs_metrics );
    createTreeEnumeration<Metric>( rhs_metrics );

    bool     m_equal = true;
    unsigned l       = 0, r = 0;
    unsigned lsize   = lhs_metrics.size();
    unsigned rsize   = rhs_metrics.size();

    while ( l < lsize || r < rsize )
    {
//      cout << lhs_metrics[l]->get_hash_id() << " =! " << rhs_metrics[r]->get_hash_id()  << endl;
        int compared_result =
            ( lhs_metrics[ l ]->get_hash_id() ).compare( rhs_metrics[ r ]->get_hash_id() );
        if (
            compared_result == 0
            )
        {       // they are equal
            cubeMap.metm[ lhs_metrics[ l ] ] = rhs_metrics[ r ];
            cubeMap.metm[ rhs_metrics[ r ] ] = lhs_metrics[ l ];
        }
        else
    ...
    // ------------- map cnodes ----------------
//     cout << " Create cnodes mapping " << endl;
    vector<Cnode*> lhs_cnodes = lhsCube.get_root_cnodev();
    vector<Cnode*> rhs_cnodes = rhsCube.get_root_cnodev();
    createTreeEnumeration<Cnode>( lhs_cnodes );
    createTreeEnumeration<Cnode>( rhs_cnodes );

    bool c_equal = true;
    l     = 0, r = 0;
    lsize = lhs_cnodes.size();
    rsize = rhs_cnodes.size();

    while ( l < lsize || r < rsize )
    {
        int compared_result =
            ( lhs_cnodes[ l ]->get_hash_id() ).compare( rhs_cnodes[ r ]->get_hash_id() );
        if (
            compared_result == 0
            )
        {       // they are equal
            cubeMap.cnodem[ lhs_cnodes[ l ] ] = rhs_cnodes[ r ];
            cubeMap.cnodem[ rhs_cnodes[ r ] ] = lhs_cnodes[ l ];
//              cout << lhs_cnodes[l]->get_hash_id() << " -> " << rhs_cnodes[r]->get_hash_id()  << endl;
        }
        else
        {       // l is smaller
            c_equal &= false;
...
//     cout << " Create system mapping " << endl;
    vector<Machine*> _lhs_sys = lhsCube.get_machv();
    vector<Machine*> _rhs_sys = rhsCube.get_machv();

    vector<Sysres*> lhs_sys;
    vector<Sysres*> rhs_sys;
    for ( vector<Machine*>::iterator it = _lhs_sys.begin(); it != _lhs_sys.end(); ++it )
    {
        lhs_sys.push_back( ( Sysres* )( *it ) );
    }
    for ( vector<Machine*>::iterator it = _rhs_sys.begin(); it != _rhs_sys.end(); ++it )
    {
        rhs_sys.push_back( ( Sysres* )( *it ) );
    }

    createTreeEnumeration<Sysres>( lhs_sys );
    createTreeEnumeration<Sysres>( rhs_sys );
    ...
    {
        int compared_result =
            ( lhs_sys[ l ]->get_hash_id() ).compare( rhs_sys[ r ]->get_hash_id() );
        if (
            compared_result == 0
            )
        {       // they are equal
            if ( lhs_sys[ l ]->isThread() && rhs_sys[ r ]->isThread() )
            {
                cubeMap.sysm[ ( Thread* )lhs_sys[ l ] ] = ( Thread* )rhs_sys[ r ];
                cubeMap.sysm[ ( Thread* )rhs_sys[ r ] ] = ( Thread* )lhs_sys[ l ];
//                  cout << lhs_sys[l]->get_hash_id() << " -> " << rhs_sys[r]->get_hash_id()  << endl;
            }
        }
        else
        {       // l is smaller
    ...
    return m_equal & c_equal & s_equal;
}
    ...
    for ( std::vector< Metric*>::const_iterator m_iter = _mets.begin(); m_iter != _mets.end(); ++m_iter )
    {
        ...
        for ( std::vector< Cnode*>::const_iterator c_iter = _cnodes.begin(); c_iter != _cnodes.end(); ++c_iter )
        {
            ...
            for ( std::vector< Thread*>::const_iterator t_iter = _thrds.begin(); t_iter != _thrds.end(); ++t_iter )
            {
                ...
                {
                    min_val = minCube.get_sev_adv( lhsmap.r_metm[ metric ], lhsmap.r_cnodem[ cnode ], lhsmap.r_sysm[ thread ] );
                }
                ...
                {
                    sub_val = subCube.get_sev_adv( rhsmap.r_metm[ metric ], rhsmap.r_cnodem[ cnode ], rhsmap.r_sysm[ thread ] );
                }
                if ( sub_val != NULL )
                {
                    *sub_val *= -1.;
                ...
                if ( min_val != val ) // in code above there are cases, where val and min_val and sub_val can get identical
                {
                    delete min_val;
                }
                if ( sub_val != val )
