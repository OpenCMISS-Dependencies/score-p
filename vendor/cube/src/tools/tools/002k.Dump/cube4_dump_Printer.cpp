...
#ifndef CUBE_DUMP_PRINTER_CPP
#define CUBE_DUMP_PRINTER_CPP
    ...
    if ( !selected_data )
    {
        ...
        for ( std::vector< std::string>::iterator iter = stored_data.begin(); iter != stored_data.end(); ++iter )
        {
                 ...
                 filename.compare( filename.length() - data_ext.length(), data_ext.length(),  data_ext ) == 0 )
            {
                ...
                sstr >> _met_id;
                Metric* _met = cube->get_met( _met_id );
...
Printer::dump_topology( Cartesian* topology, bool show_coordinates ) const
{
    ...
    if ( show_coordinates )
    {
        ...
        const std::multimap<const Sysres*, std::vector<long> >           coords = topology->get_cart_sys();
        std::multimap<const Sysres*, std::vector<long> >::const_iterator iter;
