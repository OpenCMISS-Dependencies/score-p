...
void
write_cube( Cube* outCube )
{
    cout << endl << "Write topo.cubex......";
    outCube->writeCubeReport( "topo" );
    cout << "done." << endl;
}
...
show_topologies( std::vector<Cartesian*> topologies )
{
    ...
    std::vector<std::string> names, namedims;
    ...
    for ( size_t i = 0; i < topologies.size(); i++ )
    {
        ...
        for ( int j = 0; j < topologies[ i ]->get_ndims(); j++ )
        {
            ...
            std::cout << topologies[ i ]->get_dimv()[ j ] << ( ( j + 1 < topologies[ i ]->get_ndims() ) ? " x " : "," );
            num_threads = num_threads * topologies[ i ]->get_dimv()[ j ];
        }
        std::cout << " total = " << num_threads << " threads. ";
        if ( namedims.size() == 0 )
        {
            std::cout << "Dimensions are not named.";
        }
        std::cout << std::endl;
...
rename_dimensions( Cube* inCube )
{
    ...
    {
        std::cout << "Dimensions successfully [re]named" << std::endl << std::endl;
    }
}
...
\nThe output is a topo.cube[.gz] file in the current directory." );
    ...
    if ( cube_name == "" )
    {
        cerr << "ERROR: Missing file name. " << std::endl << usage;
        exit( EXIT_FAILURE );
    }
    ...
    try {
        inCube->openCubeReport( cube_name.c_str() );
...
For more information about this file, use cube_info -S <cube experiment>\n" << std::endl;
