...
// / @cond PROTOTYPES
tracefile_costs
calculate_tracefile_costs( AggrCubeMaps*,
                           const CRegionInfo&,
                           CBlacklist* );
...
public:
    ...
    SortByMetricI( vector<metric_data<double> >& m,
                   const vector<Region*>&        r,
                   int                           anI )
...
void
setUpBlackList( string        blackListName,
...
void
setUpMetricData( string                        metriclist,
...
public:
    virtual double
    getValue( metric_data<double>& metric ) = 0;
    virtual unsigned long long
    getMaxTbcValue( tracefile_costs& c ) = 0;
    virtual unsigned long long
    getTotalTbcValue( tracefile_costs& c ) = 0;
    virtual string
    getName() = 0;
    virtual string
    getShortName() = 0;
...
public:
    double
    getValue( metric_data<double>& metric )
    {
        return metric.mpi;
    }

    unsigned long long
    getMaxTbcValue( tracefile_costs& c )
    {
        return c.max_mpi;
    }

    unsigned long long
    getTotalTbcValue( tracefile_costs& c )
    {
        return c.acc_mpi;
    }
 ...
 */
void
outMetric( IOutStrategy* str, vector<metric_data<double> >& metricData, tracefile_costs& c, bool printFooter = true, bool printHeader = true )
{
    if ( printHeader )
    {
...
/*        if (!in) {
        ...
        if ( do_regions )
        {
            ...
            for ( size_t k = 0; k < regions.size(); k++ )
            {
                ...
                if ( tbcosts.acc_costs_by_region[ i ] == 0 )
                {
                    continue;
                }
                ...
                if ( !do_blacklist || !( *blacklist )( ( Region* )regions[ i ] ) )
                {
                    ...
                    if ( ( reginfo[ i ] == EPK ) || ( reginfo[ i ] == MPI ) || ( reginfo[ i ] == OMP ) )
                    {
                        cout << " #";
                    }
                }
 ...
 */
tracefile_costs
calculate_tracefile_costs( AggrCubeMaps* input, const CRegionInfo& reginfo, CBlacklist* blacklist )
{
    const vector<Region*>& regions = input->get_regv();
