...
#ifndef CUBE_RO_Z_ROWS_SUPPLIER_CPP
#define CUBE_RO_Z_ROWS_SUPPLIER_CPP
...
ROZRowsSupplier::initSubIndex()
{
    ...
    endianess->trafo( ( char* )&subindex_size, sizeof( subindex_size ) );
    ...
    {
        endianess->trafo( ( char* )&( _sub_index[ i ].named.start_uncompressed ), sizeof( uint64_t ) );
        endianess->trafo( ( char* )&( _sub_index[ i ].named.start_compressed ),   sizeof( uint64_t ) );
        endianess->trafo( ( char* )&( _sub_index[ i ].named.size_compressed ),    sizeof( uint64_t ) );
...
void
ROZRowsSupplier::dropRow( row_t _row, cnode_id_t row_id )
{
    delete[] _row; // remove row from memory, no saving in file ß read only (because of compression)
}
...
ROZRowsSupplier::probe( fileplace_t _dataplace,
                        fileplace_t _indexplace )
{
    DataMarker* _dm      = new ZDataMarker();
    FILE*       datafile = NULL;
    __FOPEN( datafile,  _dataplace.first.c_str(), "r" );
    if ( datafile == NULL )
    {
        delete _dm;
        return false;
...
void
ROZRowsSupplier::__temp_print_row( row_t _row )
{
    if ( _row == NULL )
    {
        cout << "0xNULL" << endl;
...
void
ROZRowsSupplier::__temp_print_sub_index()
{
    cout << " --------------- SUBINDEX -------------------- " << endl;
