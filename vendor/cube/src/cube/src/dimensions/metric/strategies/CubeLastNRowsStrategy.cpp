...
LastNRowsStrategy::initialize( rows_t* _rows )
{
    ...
    removal_frontier = m_rowsInMemory.begin();
    for ( cnode_id_t _cid = 0; _cid < _rows->size(); ++_cid )
    {
...
void
LastNRowsStrategy::addRow( const cnode_id_t rowId, bool& readAllRows, std::vector<cnode_id_t>& rowsToRemove )
{
...
void
LastNRowsStrategy::removeRows( std::vector<cnode_id_t>& rowsWantToRemove, std::vector<cnode_id_t>& rowsToRemove )
{
