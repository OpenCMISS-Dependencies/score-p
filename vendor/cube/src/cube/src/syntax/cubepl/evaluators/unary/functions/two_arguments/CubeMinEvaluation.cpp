...
#ifndef __MIN_EVALUATION_CPP
#define __MIN_EVALUATION_CPP 0
...
MinEvaluation::eval()
{
    ...
    cerr << " Min of else than 2 parameters is not defined" << endl;
    return 0.;
}
...
double
MinEvaluation::eval( cube::Cnode* _cnode, CalculationFlavour _cf, cube::Sysres* _sf, CalculationFlavour _tf   )
{
    if ( getNumOfParameters() == 0 )
    {
        return 0;
    ...
    if ( getNumOfParameters() == 2 )
    {
        double _value = std::min( arguments[ 0 ]->eval(    _cnode, _cf,
                                                           _sf, _tf
                                                           ),
...
double
MinEvaluation::eval( cube::Cnode* _cnode, CalculationFlavour _cf )
{
    if ( getNumOfParameters() == 0 )
    {
        return 0;
...
double
MinEvaluation::eval( double arg1, double arg2 )
{
    if ( getNumOfParameters() == 0 )
    {
        return 0;
    ...
    if ( getNumOfParameters() == 2 )
    {
        double _value = std::min( arguments[ 0 ]->eval( arg1, arg2  ),
