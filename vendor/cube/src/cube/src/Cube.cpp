                       ...
                       bool        write_ghost_metrics )
{
    ...
    catch ( const FinalizeFilesError& err )
    {
               ...
               )
{
    return def_met( disp_name, uniq_name, dtype, uom, val, url,
                    descr, parent, cur_metric_id, type_of_metric, expression, init_expression, aggr_plus_expression, aggr_minus_expression, threadwise, is_ghost );
}
    ...
    switch ( type_of_metric )
    {
        case CUBE_METRIC_POSTDERIVED:
        {
            ...
            met = new PostDerivedMetric( disp_name, uniq_name,  parent_dtype, uom, val, url,
                                         descr, filefinder, parent, id, expression, init_expression, threadwise, is_ghost );
        ...
        case CUBE_METRIC_PREDERIVED_INCLUSIVE:
        {
            ...
            if ( aggr_plus_expression != "" || aggr_minus_expression != "" )
            {
                met = new CustomPreDerivedInclusiveMetric( disp_name, uniq_name,  parent_dtype, uom, val, url,
                                                           descr, filefinder, parent, id, expression, init_expression, aggr_plus_expression, aggr_minus_expression, threadwise, is_ghost  );
            }
            else
            {
        ...
        case CUBE_METRIC_PREDERIVED_EXCLUSIVE:
        {
            ...
            if ( aggr_plus_expression != "" )
            {
                met = new CustomPreDerivedExclusiveMetric( disp_name, uniq_name, parent_dtype, uom, val, url,
                                                           descr, filefinder, parent, id, expression, init_expression, aggr_plus_expression, threadwise, is_ghost  );
            }
            else
            {
        ...
        case CUBE_METRIC_EXCLUSIVE:
            if (   ( strcmp( dtype.c_str(), "FLOAT" ) == 0
                     ||
                     strcmp( dtype.c_str(), "DOUBLE" ) == 0 ) )
            {
                met = new ExclusiveBuildInTypeMetric<double>( disp_name, uniq_name, dtype, uom, val, url,
                                                              descr, filefinder, parent, id, is_ghost  );
            }
            else
            if (   ( strcmp( dtype.c_str(), "INT64" ) == 0
                     ||
                     strcmp( dtype.c_str(), "INTEGER" ) == 0 ) )
            {
                met = new ExclusiveBuildInTypeMetric<int64_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                               descr, filefinder, parent, id, is_ghost  );
            }
            else
            if ( strcmp( dtype.c_str(), "UINT64" ) == 0 )
            {
                met = new ExclusiveBuildInTypeMetric<uint64_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                                descr, filefinder, parent, id, is_ghost  );
            }
            else
            if ( strcmp( dtype.c_str(), "UINT32" ) == 0 )
            {
                met = new ExclusiveBuildInTypeMetric<uint32_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                                descr, filefinder, parent, id, is_ghost  );
            }
            else
            if ( strcmp( dtype.c_str(), "INT32" ) == 0 )
            {
                met = new ExclusiveBuildInTypeMetric<int32_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                               descr, filefinder, parent, id, is_ghost  );
            }
            else
            if ( strcmp( dtype.c_str(), "UINT16" ) == 0 )
            {
                met = new ExclusiveBuildInTypeMetric<uint16_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                                descr, filefinder, parent, id, is_ghost  );
            }
            else
            if ( strcmp( dtype.c_str(), "INT16" ) == 0 )
            {
                met = new ExclusiveBuildInTypeMetric<int16_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                               descr, filefinder, parent, id, is_ghost  );
            }
            else
            if ( strcmp( dtype.c_str(), "UINT8" ) == 0 )
            {
                met = new ExclusiveBuildInTypeMetric<uint8_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                               descr, filefinder, parent, id, is_ghost  );
            }
            else
            if ( strcmp( dtype.c_str(), "INT8" ) == 0 )
            {
                met = new ExclusiveBuildInTypeMetric<int8_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                              descr, filefinder, parent, id, is_ghost  );
            }
        ...
        default:
            if (   ( strcmp( dtype.c_str(), "FLOAT" ) == 0
                     ...
                     strcmp( dtype.c_str(), "DOUBLE" ) == 0 ) )
            {
                met = new InclusiveBuildInTypeMetric<double>( disp_name, uniq_name, dtype, uom, val, url,
                                                              descr, filefinder, parent, id, is_ghost  );
            }
            else
            if (   ( strcmp( dtype.c_str(), "INT64" ) == 0
                     ...
                     strcmp( dtype.c_str(), "INTEGER" ) == 0 ) )
            {
                met = new InclusiveBuildInTypeMetric<int64_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                               descr, filefinder, parent, id, is_ghost  );
            }
            else
            if ( strcmp( dtype.c_str(), "UINT64" ) == 0 )
            {
                met = new InclusiveBuildInTypeMetric<uint64_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                                descr, filefinder, parent, id, is_ghost  );
            }
            else
            if ( strcmp( dtype.c_str(), "UINT32" ) == 0 )
            {
                met = new InclusiveBuildInTypeMetric<uint32_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                                descr, filefinder, parent, id, is_ghost  );
            }
            else
            if ( strcmp( dtype.c_str(), "INT32" ) == 0 )
            {
                met = new InclusiveBuildInTypeMetric<int32_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                               descr, filefinder, parent, id, is_ghost  );
            }
            else
            if ( strcmp( dtype.c_str(), "UINT16" ) == 0 )
            {
                met = new InclusiveBuildInTypeMetric<uint16_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                                descr, filefinder, parent, id, is_ghost  );
            }
            else
            if ( strcmp( dtype.c_str(), "INT16" ) == 0 )
            {
                met = new InclusiveBuildInTypeMetric<int16_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                               descr, filefinder, parent, id, is_ghost  );
            }
            else
            if ( strcmp( dtype.c_str(), "UINT8" ) == 0 )
            {
                met = new InclusiveBuildInTypeMetric<uint8_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                               descr, filefinder, parent, id, is_ghost  );
            }
            else
            if ( strcmp( dtype.c_str(), "INT8" ) == 0 )
            {
                met = new InclusiveBuildInTypeMetric<int8_t>( disp_name, uniq_name, dtype, uom, val, url,
                                                              descr, filefinder, parent, id, is_ghost  );
            }
            else                                                                                // use general implementation
            {
...
Metric*
Cube::get_met( uint32_t _id ) const
{
    for ( std::vector<Metric*>::const_iterator iter = metv.begin(); iter != metv.end(); ++iter )
    {
        if ( *iter == NULL )
...
Cube::writeXML_header( ostream& out, bool cube3_export, bool write_ghost_metrics )
{
    ...
    if ( !cube3_export )
    {
        def_attr( CUBE_LIBRARY_VERSION_KEY, CUBE_FULL_NAME " r" CUBE_REVISION_STRING );
        def_attr( CUBEPL_VERSION_KEY, CUBEPL_VERSION );
        def_attr( CUBE_VERSION_KEY, CUBE_VERSION );
        def_attr( CUBE_ANCHOR_VERSION_KEY, CUBE_ANCHOR_VERSION );
    }
    ...
    {
        if ( (
                 ai->first.compare( CUBE_LIBRARY_VERSION_KEY ) == 0
                 ||
                 ai->first.compare( CUBEPL_VERSION_KEY ) == 0
                 ||
                 ai->first.compare( CUBE_VERSION_KEY ) == 0
                 ||
                 ai->first.compare( CUBE_ANCHOR_VERSION_KEY ) == 0
                 )
             && cube3_export )
        {
            continue;
...
operator<<( ostream& out,  Cube& cube )
{
    ...
    catch ( const Cube3SystemTreeMismatchError& e )
    {
        cerr << e.get_msg();
        throw e;
    }
