...
{
/**
 * IDassigned is used by cube(metric) to assign id for the treelike objects.
...
//     std::map<unsigned, signed int> calltree_strides;           ///< (ABSOLETE ... ) Optimisation trick. Saves the range of the children, which one takes to calculate Inc/excl Value for every element in the tree as a map ID -> stride.
    ...
    traverse( row_of_objects_t*,
              cube::Vertex* );                                               // /< Every type of enumerator defines its way to run over the objects and to assign its id. Might be recursive call.
};
}
