...
public:
 ...
 /*   RowWiseMatrix( fileplace_t,
    ...
    virtual Value* sumRow( cnode_id_t ); // /< sum over full row
    ...
    template <class T>
    T
    getBuildInValue( cnode_id_t, thread_id_t );

    template <class T>
    T
    sumBuildInTypeRow( cnode_id_t, thread_id_t, uint64_t  );

    template <class T>
    T
    sumBuildInTypeRow( cnode_id_t );
};
...
template <class T>
T
RowWiseMatrix::sumBuildInTypeRow( cnode_id_t cid, thread_id_t tid, uint64_t number )
{
    ...
    return row_interface->template sumBuildInTypeRow<T>( rows[ cid ], tid, number );
}
...
template <class T>
T
RowWiseMatrix::sumBuildInTypeRow( cnode_id_t cid )
{
    ...
    T _v = row_interface->template sumBuildInTypeRow<T>( __row );
    return _v;
}
}
