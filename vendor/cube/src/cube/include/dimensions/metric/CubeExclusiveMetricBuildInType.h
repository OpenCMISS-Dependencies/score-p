...
public:
    ...
    ExclusiveBuildInTypeMetric<T>( const std::string & disp_name,
...
ExclusiveBuildInTypeMetric<T>::get_sev( Cnode* cnode, CalculationFlavour cnf, Sysres* sys, CalculationFlavour sf )
{
    ...
    if ( sys->isLocationGroup() )
    {
        ...
        {
            T _v = this->get_sev( cnode, cnf, _lg->get_child( i ), cube::CUBE_CALCULATE_INCLUSIVE );
            ( v ) += _v;
        }
    }
    if ( sys->isLocation() )
    {
        ...
        for ( std::vector<Cnode*>::const_iterator _iter = _cnodes.begin(); _iter != _cnodes.end(); ++_iter )
        {
            ...
            T              _v      = ( this->adv_sev_mat )->template getBuildInValue<T>( this->calltree_local_ids[ __c->get_remapping_cnode( lg_rank  )->get_id() ], _loc->get_id() );
