...
#ifndef __CUBEPL2_MEMORY_MANAGER_H
#define __CUBEPL2_MEMORY_MANAGER_H 0
...
private:
    ...
    std::vector< std::vector<CubePL2MemoryDuplet> > memory;              // normal memory. Every new expression calculation allocates new "page" and moves stack pointer
...
public:
    ...
    virtual
    size_t
        size_of( CubePL2MemoryAdress, CubePL2KindOfVariable = CUBEPL_VARIABLE );
    virtual
    bool
    defined( std::string  );
    ...
    CubePL2StateOfCubePLVariable
        type( MemoryAdress, double, KindOfVariable );
};
}
