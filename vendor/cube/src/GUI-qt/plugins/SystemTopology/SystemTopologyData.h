...
private:
    ...
    std::vector<std::vector<std::vector<TreeItem*> > > coordToItem;
    // assignment of displayed coordinates to items, an item may have multiple coordinates
    QHash<TreeItem*, std::vector<std::vector<long> > > itemToCoord;
    // if more than 3 dimensions: assignment of complete coordinates to items
    QHash<TreeItem*, const std::vector<long>* > itemToNDCoord;
