...
ValueWidget::paintEvent( QPaintEvent* )
{
    ...
    if ( selectedValueExists )
    {
        ...
        if ( percent != 0.0 )
        {
            percent = 100.0 * ( selectedValue - minValue ) / percent;
        }
        else
        {
            percent = 100.0;
    ...
    if ( selectedValueExists )
    {
        selectedValueString.append( Globals::formatNumber( selectedValue, intType1, FORMAT_DEFAULT ) );
    ...
    if ( absValueExists )
    {
        ...
        else
        {
            ...
            absValueString.append( " +/- " );
            absValueString.append( Globals::formatNumber( variance, intType1, FORMAT_DEFAULT ) );
        }
...
void
ValueWidget::update( double minValue, double maxValue, double selectedValue, double minAbsValue, double maxAbsValue, double absValue, bool intType, bool userDefinedMinMaxValues, double _mean, double _variance )
{
...
void
ValueWidget::update( double minValue, double maxValue, double selectedValue, bool intType, bool userDefinedMinMaxValues, double _mean, double _variance )
{
    this->minValue                = minValue;
    this->maxValue                = maxValue;
    minMaxValuesExist             = true;
...
void
ValueWidget::update( double minValue, double maxValue, bool intType, bool userDefinedMinMaxValues, double _mean, double _variance )
{
    this->minValue                = minValue;
    this->maxValue                = maxValue;
