...
{
    cw = new ColorWidget();
    connect( cw, SIGNAL( colorsChanged() ), this, SLOT( onColorMapChanged() ) );
}
...
QColor
DefaultColorMap::getColor( double value, double minValue, double maxValue, bool whiteForZero ) const
{
    return cw->getColor( value, minValue, maxValue, whiteForZero );
}

QString
DefaultColorMap::getMapName() const
{
    return "Default colormap";
}

bool
DefaultColorMap::dialogAvailable() const
{
    return true;
}

void
DefaultColorMap::showDialog()
{
    cw->setColors();
}

void
DefaultColorMap::onColorMapChanged()
{
    emit colorMapChanged();
}

void
DefaultColorMap::loadGlobalSettings( QSettings& settings )
{
    // restore coloring
    cw->setLightenStart( settings.value( "color/lighten", 0.0 ).toDouble() );
    cw->setWhiteStart( settings.value( "color/white", 0.0 ).toDouble() );
    cw->setColorPos( 0, settings.value( "color/pos0", 0.0 ).toDouble() );
    cw->setColorPos( 1, settings.value( "color/pos1", 0.1 ).toDouble() );
    cw->setColorPos( 2, settings.value( "color/pos2", 0.2 ).toDouble() );
    cw->setColorPos( 3, settings.value( "color/pos3", 0.3 ).toDouble() );
    cw->setColorPos( 4, settings.value( "color/pos4", 1.0 ).toDouble() );
    cw->setColoringMethod( ( ColoringMethod )( settings.value( "color/method", EXPONENTIAL2 ).toInt() ) );
    cw->update();
    cw->update();
}

void
DefaultColorMap::saveGlobalSettings( QSettings& settings )
{
    // save coloring
    settings.setValue( "color/lighten", cw->getLightenStart() );
    settings.setValue( "color/white", cw->getWhiteStart() );
    settings.setValue( "color/pos0", cw->getColorPos( 0 ) );
    settings.setValue( "color/pos1", cw->getColorPos( 1 ) );
    settings.setValue( "color/pos2", cw->getColorPos( 2 ) );
    settings.setValue( "color/pos3", cw->getColorPos( 3 ) );
    settings.setValue( "color/pos4", cw->getColorPos( 4 ) );
    settings.setValue( "color/method", cw->getColoringMethod() );
}
...
void
ColorWidget::setLightenStart( double value )
{
    lightenStart = value;
}
void
ColorWidget::setWhiteStart( double value )
{
    whiteStart = value;
}
void
ColorWidget::setColoringMethod( ColoringMethod coloringMethod )
{
    this->coloringMethod = coloringMethod;
}
void
ColorWidget::setInDialog( bool inDialog )
{
...
//
void
ColorWidget::onApply( ColorWidget* _colorWidget )
{
...
//
void
ColorWidget::setColorPos( unsigned index, double value )
{
    if ( value < 0.0 )
    {
        value = 0.0;
...
double
ColorWidget::getLightenStart()
{
    return lightenStart;
}
double
ColorWidget::getWhiteStart()
{
    return whiteStart;
}
double
ColorWidget::getColorPos( unsigned index )
{
    return colorPos[ index ];
}
ColoringMethod
ColorWidget::getColoringMethod()
{
    return coloringMethod;
}
bool
ColorWidget::getInDialog()
{
    return inDialog;
}
...
void
ColorWidget::mouseReleaseEvent( QMouseEvent* event )
{
    event->accept();
    mousePressed = false;
}
...
void
ColorWidget::mouseMoveEvent( QMouseEvent* event )
{
    event->accept();
    if ( mousePressed )
    {
...
//
void
ColorWidget::setMouseColorPos( QMouseEvent* event )
{
    int x = event->pos().x();
    int y = event->pos().y();
    int w = width();
...
//
void
ColorWidget::updateColorPos( QMouseEvent* event )
{
    int x = event->pos().x();
    int w = width();
...
//
QColor
ColorWidget::lighten( QColor color, double position )
{
    if ( lightenStart <= position )
    {
        return color;
    }
...
//
QColor
ColorWidget::getColor( double value, double minValue, double maxValue, bool whiteForZero )
{
...
double
ColorWidget::linIncr( double min, double max, double value )
{
    if ( min == max )
    {
        return 255.0;
    ...
    {
        return 255.0 * ( value - min ) / ( max - min );
    }
}
double
ColorWidget::linDecr( double min, double max, double value )
{
    return linIncr( min, max, max - ( value - min ) );
}

double
ColorWidget::quadratic1Incr( double min, double max, double value )
{
    if ( min == max )
    {
        return 255.0;
...
double
ColorWidget::quadratic1Decr( double min, double max, double value )
{
    return quadratic1Incr( min, max, max - ( value - min ) );
}

double
ColorWidget::quadratic2Incr( double min, double max, double value )
{
    if ( min == max )
    {
        return 255.0;
...
double
ColorWidget::quadratic2Decr( double min, double max, double value )
{
    return quadratic2Incr( min, max, max - ( value - min ) );
}

double
ColorWidget::exp1Incr( double min, double max, double value )
{
    if ( min == max )
    {
        return 255.0;
...
double
ColorWidget::exp1Decr( double min, double max, double value )
{
    return exp1Incr( min, max, max - ( value - min ) );
}

double
ColorWidget::exp2Incr( double min, double max, double value )
{
    if ( min == max )
    {
        return 255.0;
...
double
ColorWidget::exp2Decr( double min, double max, double value )
{
    return exp2Incr( min, max, max - ( value - min ) );
}

double
ColorWidget::incr( double min, double max, double value )
{
    if ( coloringMethod == LINEAR )
    {
        return linIncr( min, max, value );
    }
    else if ( coloringMethod == QUADRATIC1 )
    {
        return quadratic1Incr( min, max, value );
    }
    else if ( coloringMethod == QUADRATIC2 )
    {
        return quadratic2Incr( min, max, value );
    }
    else if ( coloringMethod == EXPONENTIAL1 )
    {
        return exp1Incr( min, max, value );
    }
    else
    {
        return exp2Incr( min, max, value );
    }
}

double
ColorWidget::decr( double min, double max, double value )
{
    if ( coloringMethod == LINEAR )
    {
        return linDecr( min, max, value );
    }
    else if ( coloringMethod == QUADRATIC1 )
    {
        return quadratic1Decr( min, max, value );
    }
    else if ( coloringMethod == QUADRATIC2 )
    {
        return quadratic2Decr( min, max, value );
    }
    else if ( coloringMethod == EXPONENTIAL1 )
    {
        return exp1Decr( min, max, value );
    }
...
ColorDialog::ColorDialog( QWidget*     parent,
                          ColorWidget* _colorWidget ) : QDialog( parent )
{
    ...
    if ( colorWidget->getColoringMethod() == LINEAR )
    {
        button1->setChecked( true );
    }
    else if ( colorWidget->getColoringMethod() == QUADRATIC1 )
    {
        button2->setChecked( true );
    }
    else if ( colorWidget->getColoringMethod() == QUADRATIC2 )
    {
        button3->setChecked( true );
    }
    else if ( colorWidget->getColoringMethod() == EXPONENTIAL1 )
    {
...
void
ColorDialog::onLinear( bool activ )
{
    if ( activ )
    {
        colorWidget->setColoringMethod( LINEAR );
    }
    update();
}
void
ColorDialog::onQuadratic1( bool activ )
{
    if ( activ )
    {
        colorWidget->setColoringMethod( QUADRATIC1 );
    }
    update();
}
void
ColorDialog::onQuadratic2( bool activ )
{
    if ( activ )
    {
        colorWidget->setColoringMethod( QUADRATIC2 );
    }
    update();
}
void
ColorDialog::onExponential1( bool activ )
{
    if ( activ )
    {
        colorWidget->setColoringMethod( EXPONENTIAL1 );
    }
    update();
}
void
ColorDialog::onExponential2( bool activ )
{
    if ( activ )
    {
        colorWidget->setColoringMethod( EXPONENTIAL2 );
    }
    update();
}

void
ColorDialog::setLightenStart( double value )
{
    colorWidget->setLightenStart( value / 100.0 );
    update();
}
void
ColorDialog::setWhiteStart( double value )
{
    colorWidget->setWhiteStart( value / 100.0 );
    update();
}

void
ColorDialog::setColorPos0( double value )
{
    setColorPos( 0, value );
}
void
ColorDialog::setColorPos1( double value )
{
    setColorPos( 1, value );
}
void
ColorDialog::setColorPos2( double value )
{
    setColorPos( 2, value );
}
void
ColorDialog::setColorPos3( double value )
{
    setColorPos( 3, value );
}
void
ColorDialog::setColorPos4( double value )
{
    setColorPos( 4, value );
}

void
ColorDialog::setColorPos( unsigned index, double value )
{
...
void
ColorDialog::onUpdateValueSpins()
{
    for ( int i = 0; i < 5; i++ )
    {
