 ...
 */
#ifndef __CUBEW_SERVICES_C
#define __CUBEW_SERVICES_C 0
#include <config.h>
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
...
__cube_metric_size_dtype( cube_value_param_array* dtype )
{
    ...
    fprintf( stderr, "[CUBEW Warning]: Selected type of values (%u) in metric is unknown. Return size of it as 0\n", ( unsigned )( dtype->type ) );
    return 0;
}
...
__cube_metric_dtype2string( cube_value_param_array* dtype )
{
    ...
    if ( dtype->type == CUBE_DATA_TYPE_HISTOGRAM )
    {
        ...
        char* _string = ( char* )calloc( len, 1 );
    ...
    fprintf( stderr, "[CUBEW Warning]: Selected type of values (%u) in metric is unknown. Return \"UNKNOWN\" name\n", ( unsigned )( dtype->type ) );
    return "UNKNOWN";
}
...
{
    cube_value_param_array* _type = ( cube_value_param_array* )calloc( sizeof( cube_value_param_array ), 1 );
    ...
    while ( token != NULL )
    {
        ...
        ADD_NEXT( _type, _value, uint32_t );
    }
    free( total_dtype );
    return _type;
}
                            ...
                            int      cid )
{
    ...
    if ( vlen * 8 < cid )
    {
        ...
        fprintf( stderr, "[CUBEW Fatal Error]: Position %d is out of bit vector with length %u. Resulted Cube file will be invalid.\n", cid, vlen );
        exit( -1 );
    }
    ;
...
__cube_metric_size_of_index( char*    bit_vector,
                             unsigned vlen )
{
    return __cube_metric_bit_position( bit_vector, vlen, vlen * 8 );  /*  position of the last bit... */
}
                            ...
                            unsigned vlen )
{
    ...
    uint32_t* index = ( uint32_t* )malloc( isize * sizeof( uint32_t ) );
                                 ...
                                 )
{
    ...
    position -= bits[ ( unsigned )( bit_vector[ i ] ) ];
    return position;
}
...
__cube_transform_doubles_into_uint64( double*  data_row,
                                      uint32_t size )
{
    uint64_t* target_row = ( uint64_t* )malloc( size * sizeof( uint64_t ) );
    unsigned  i          = 0;
    for ( i = 0; i < size; i++ )
    {
...
__cube_transform_doubles_into_int64( double*  data_row,
                                      uint32_t size )
{
    int64_t* target_row = ( int64_t* )malloc( size * sizeof( int64_t ) );
    unsigned  i          = 0;
    for ( i = 0; i < size; i++ )
    {
...
__cube_transform_uint64_into_doubles( uint64_t* data_row,
                                      uint32_t  size )
{
    double*  target_row = ( double* )malloc( size * sizeof( double ) );
    unsigned i          = 0;
    for ( i = 0; i < size; i++ )
    {
...

{
    ...
    void* target_row = calloc( size,  to_sizeof );
...
void
__cube_value_transform( cube_value_param_array* from, cube_value_param_array* to, char* target_place_for_value, void* source, uint32_t to_sizeof, uint32_t from_sizeof )
{
    ...
    switch ( from->type )
    {
        ...
        default:
            fprintf( stderr, "Type casting from UNKNOWN, TAU_ATOMIC, COMPLEX, SCALE_FUNC, RATE or HISTOGRAM type is not supported\n" );
            break;
    }
    switch ( to->type )
    {
        ...
        case CUBE_DATA_TYPE_HISTOGRAM:
        default:
            fprintf( stderr, "Type casting to UNKNOWN, TAU_ATOMIC, COMPLEX, SCALE_FUNC, RATE or HISTOGRAM type is not supported\n" );
            break;
    }
}
...
__cube_type_compatibility_check( cube_value_param_array* dtype_params_from, cube_value_param_array* dtype_params_to )
{
    ...
    fprintf( stderr, "[ERROR] Relation of %s values with %s values is not unknown.\n", __cube_metric_dtype2string( dtype_params_from ), __cube_metric_dtype2string( dtype_params_to ) );
...
__cube_initialize_row_of_memory( cube_value_param_array* dtype_params, enum value_init_t sort_of_value, void* row, uint64_t size, void* value )
{
    ...
    {
        fprintf( stderr, "Initialization of memory with *NULL value is not posible. Leave uninitialized.\n" );
    }
}
...
__cube_replace_symbols2( char* str )
{
    ...
    if ( toks )
    {
        r = ( char* )calloc( len + ( toks * 6 ), 1 ); /* allocate sufficiently long string */
    }
    p = s;
...
void
__cube_services_write_attributes( cube_meta_data_writer* writer,  cube_att_array* attr, const char* ind1, const char* ind2 )
{
    /* metadata info */
    if ( attr == NULL )
    {
    ...
    for ( i = 0; i < attr->size; i++ )
    {
        ...
        __cube_write_meta_data( writer, "%s%s  <attr key=\"%s\" value=\"%s\"/>\n", ind2, ind1, _key ? _key : key,  _value ? _value : value );
        if ( _key )
        {
            free( _key );
        }
