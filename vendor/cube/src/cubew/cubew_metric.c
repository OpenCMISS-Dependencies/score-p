                  ...
                  enum bool_t           compressed )
{
    ...
    {
        fprintf( stderr, "Derived metrics support only DOUBLE or FLOAT as a datatype. Enforce DOUBLE datatype.\n" );
                                 ...
                                 char*                   expression )
{
    ...
    {
        fprintf( stderr, "WARNING: Only prederived metrics do specify an aggregation. Ignore." );
        return;
    }
    ...
    else
    {
        ...
        {
            fprintf( stderr, "WARNING: Only prederived inclusive metrics do specify an aggregation for minus operation. Ignore." );
        }
    }
}
                                ...
                                cube_dyn_array* cnodes )
{
    ...
    {
        if ( this->local_cnode_enumeration->data != NULL )
        {
            free( this->local_cnode_enumeration->data );
            this->local_cnode_enumeration->size = 0;
        }
    }
    else
    {
        ALLOC( this->local_cnode_enumeration, 1, cube_dyn_array );
    }

    if ( this->optimal_cnodes_sequence != NULL )
    {
        if ( this->optimal_cnodes_sequence->data != NULL )
        {
            free( this->optimal_cnodes_sequence->data );
            this->local_cnode_enumeration->size = 0;
        }
    }
    else
    {
        ALLOC( this->optimal_cnodes_sequence, 1, cube_dyn_array );
    }

    if ( this->optimal_cnodes_sequence_with_cndptrs != NULL )
    {
        if ( this->optimal_cnodes_sequence_with_cndptrs->data != NULL )
        {
            free( this->optimal_cnodes_sequence_with_cndptrs->data );
        }
    }
    else
    {
        ALLOC( this->optimal_cnodes_sequence_with_cndptrs, 1, carray );
    }
    ...
    ALLOC( this->local_cnode_enumeration->data, this->ncn, void* );
    ALLOC( this->optimal_cnodes_sequence->data, this->ncn, void* );
    ALLOC( this->optimal_cnodes_sequence_with_cndptrs->data, this->ncn,  cube_cnode* );
                                                ...
                                                cube_cnode*  cnode )
{
    if ( *index >= size )
    {
        fprintf( stderr, "WARNING: Traversation index %u became bigger than size %u. Stop traversing (deep search)", ( unsigned )( *index ), size );
        return;
    }
    map[ cnode->id ]          = *index;
                                                ...
                                                cube_cnode*  cnode )
{
    ...
    if ( ( *index ) > size )
    {
        fprintf( stderr, "WARNING: Traversation index %u became bigger than size %u. Stop traversing (wide search)", *index, size );
        return;
    }
                                        ...
                                        cube_cnode*  cnode )
{
    if ( *index >= size )
    {
        fprintf( stderr, "WARNING: Traversation index %u became bigger than size %u. Stop traversing (simple)", *index, size );
        return;
    }
    map[ cnode->id ]          = *index;
    sequence[ *index ]        = cnode->id;
...
cube_metric_free( cube_metric* this )
{
    ...
    free( this->sub_index );
#if defined( BACKEND_CUBE_COMPRESSED ) || defined( FRONTEND_CUBE_COMPRESSED )
    free( this->compression_buffer );
#endif /* HAVE_LIB_Z */
    free( this->known_cnodes );
    free( this->written_cnodes );
    int i = 0;
    if ( this->attr )
    {
    ...
    {
        if ( this->local_cnode_enumeration->data != 0 )
        {
            free( this->local_cnode_enumeration->data );
        }
        free( this->local_cnode_enumeration );
    }
    if ( this->optimal_cnodes_sequence != 0 )
    {
        if ( this->optimal_cnodes_sequence->data != 0 )
        {
            free( this->optimal_cnodes_sequence->data );
        }
        free( this->optimal_cnodes_sequence );
    }
    if ( this->optimal_cnodes_sequence_with_cndptrs != 0 )
    {
        if ( this->optimal_cnodes_sequence_with_cndptrs->data != 0 )
        {
            free( this->optimal_cnodes_sequence_with_cndptrs->data );
        }
        free( this->optimal_cnodes_sequence_with_cndptrs );
    }
    free( this->expression );
    free( this->init_expression );
    free( this->aggr_plus_expression );
    free( this->aggr_minus_expression );
                                 ...
                                 unsigned     cid )                           // position in  file without marker. (as if there wouldn't be any markers at the beginning)
{
    ...
    if ( metric->metric_format == CUBE_INDEX_FORMAT_SPARSE )
    {
        ...
        {
            fprintf( stderr, "[CUBEW Warning]: Parallel writing of cube with sparse metric is possible only with set bit vector.\n" );
            fprintf( stderr, "[CUBEW Warning]: Format of metric is changed to DENSE.\n" );
...
//          ((unsigned*)(metric->optimal_cnodes_sequence->data))[cid]);
            return _position * ( metric->nthrd );
        }
    }
    fprintf( stderr, "[CUBEW Warning]: Metric binary format is neither DENSE nor SPARSE. Should be error. Return as position %u.\n", _position );
    return _position;
}
                              ...
                              char*        known_cnodes /*, unsigned size*/ )
{
    ...
    if ( known_cnodes == 0 )
    {
        fprintf( stderr, "Failed to set a bit vector of known cnodes. Recieved pointer is zero.\n" );
    }
    free( metric->known_cnodes );
...
cube_metric_setup_subindex( cube_metric* metric )
{
    if ( metric->compressed == CUBE_TRUE )
    {
        metric->n_compressed = ( metric->known_cnodes == 0 ) ? metric->ncn :
                               __cube_metric_size_of_index( metric->known_cnodes, ( unsigned )ceil( (
                                                                                                        double )metric->ncn / 8. ) );
        ...
        free( metric->sub_index );
        metric->sub_index = ( uint64_t* )calloc(  metric->subi_size, 1 );
        memset( metric->sub_index, 0,   metric->subi_size );

        free( metric->compression_buffer );
        metric->compression_buffer = ( Bytef* )calloc( metric->nthrd * __cube_metric_size_dtype( metric->dtype_params ) + 512, 1 );     //  512 bytes bigger than origina.l. For the case of empry source , compressed buffer is bigger
        memset( metric->compression_buffer, 0,  metric->nthrd * __cube_metric_size_dtype( metric->dtype_params ) + 512 );
    }
}
#endif
                      ...
                      cube_meta_data_writer* meta_data_writer )
{
    ...
    if ( this->metric_viz_type == CUBE_METRIC_NORMAL )
    {
        __cube_write_meta_data( meta_data_writer, "%s    <metric id=\"%u\" type=\"%s\">\n", ind, _id, _own_type );
    }
    else
    {
        __cube_write_meta_data( meta_data_writer, "%s    <metric id=\"%u\" type=\"%s\" viztype=\"%s\">\n", ind, _id, _own_type, _own_viz_type );
    }
    __cube_write_meta_data( meta_data_writer, "%s      <disp_name>%s</disp_name>\n", ind,  _disp_name ? _disp_name : cube_metric_get_disp_name( this ) );
    __cube_write_meta_data( meta_data_writer, "%s      <uniq_name>%s</uniq_name>\n", ind, _uniq_name ? _uniq_name : cube_metric_get_uniq_name( this ) );
    __cube_write_meta_data( meta_data_writer, "%s      <dtype>%s</dtype>\n", ind,  _dtype ? _dtype : cube_metric_get_dtype( this ) );
    __cube_write_meta_data( meta_data_writer, "%s      <uom>%s</uom>\n", ind, _uom ? _uom : cube_metric_get_uom( this ) );
    if ( strcmp( cube_metric_get_val( this ), "" ) != 0 )
    {
        __cube_write_meta_data( meta_data_writer, "%s      <val>%s</val>\n", ind, _val ? _val : cube_metric_get_val( this ) );
    }
    __cube_write_meta_data( meta_data_writer, "%s      <url>%s</url>\n", ind, _url ? _url : cube_metric_get_url( this ) );
    __cube_write_meta_data( meta_data_writer, "%s      <descr>%s</descr>\n", ind, _descr ? _descr : cube_metric_get_descr( this ) );
    if ( this->metric_type == CUBE_METRIC_POSTDERIVED  ||   this->metric_type == CUBE_METRIC_PREDERIVED_INCLUSIVE  ||  this->metric_type == CUBE_METRIC_PREDERIVED_EXCLUSIVE )
    {
        if ( _expression != NULL )
        {
            ...
            {
                __cube_write_meta_data( meta_data_writer, "%s      <cubepl locationwise=false>%s</cubepl>\n", ind, _expression  );
            }
            else
            {
                __cube_write_meta_data( meta_data_writer, "%s      <cubepl>%s</cubepl>\n", ind, _expression  );
            }
        }
        if ( _init_expression != NULL )
        {
            __cube_write_meta_data( meta_data_writer, "%s      <cubeplinit>%s</cubeplinit>\n", ind, _init_expression );
        }
        if ( _aggr_plus_expression != NULL )
        {
            __cube_write_meta_data( meta_data_writer, "%s      <cubeplaggr cubeplaggrtype=plus>%s</cubeplaggr>\n", ind, _aggr_plus_expression );
        }
        if ( _aggr_minus_expression != NULL )
        {
            __cube_write_meta_data( meta_data_writer, "%s      <cubeplaggr cubeplaggrtype=minus>%s</cubeplaggr>\n", ind, _aggr_minus_expression );
        }
    }
    ...
    __cube_write_meta_data( meta_data_writer, "%s    </metric>\n", ind );
    ...
    free( _descr );
    free( _expression );
    free( _init_expression );
    free( _aggr_plus_expression );
    free( _aggr_minus_expression );
}
...
cube_metric_size_of_data_file( cube_metric* this )
{
    ...
    {
        ncnodes = __cube_metric_last_bit_position( this->known_cnodes, ( this->ncn + 7 ) / 8 );
    }
    else if ( this->written_cnodes != 0 )
    {
        ncnodes = __cube_metric_last_bit_position( this->written_cnodes, ( this->ncn + 7 ) / 8 );
    }
    else
    {
        ncnodes = this->ncn;
    ...
    if ( this->metric_format == CUBE_INDEX_FORMAT_SPARSE )
    {
        size = ncnodes * ( uint64_t )this->nthrd * ( uint64_t )( __cube_metric_size_dtype( this->dtype_params ) );         /*  INDEX of metric */
    }
                                        ...
                                        char*        known_cnodes )
{
    ...
    char*    to_return     = ( char* )calloc( bitstringsize, 1 ); /*  Allocate  same amount of memory for bitstring*/
                       ...
                       void*        data_row )
{
    ...
    {
        metric->written_cnodes = ( char* )calloc( ( unsigned )ceil( ( double )metric->ncn / 8. ), 1 );
    }
    ...
    else
    {
        ...
        compress( metric->compression_buffer,   &size_compressed, ( Bytef* )( data_row ), row_size );
                                      ...
                                      double*      data_row )
{
    ...
    switch ( metric->dtype_params->type )
    {
        ...
        case CUBE_DATA_TYPE_UINT64:
            ...
            break;
        default:
            target_row = ( void* )calloc( metric->nthrd * sizeof( double ), 1 );
                                     ...
                                     uint64_t*    data_row )
{
    ...
    switch ( metric->dtype_params->type )
    {
        ...
        case CUBE_DATA_TYPE_MAX_DOUBLE:
            ...
            break;
        default:
            target_row = ( void* )calloc( metric->nthrd * sizeof( uint64_t ), 1 );
